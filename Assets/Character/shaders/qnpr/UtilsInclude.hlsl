...
inline half4 VertexGIForward(a2v v, float3 posWorld, half3 normalWorld)
{
    half4 UVmap = 0;
    // Static lightmaps
    #ifdef LIGHTMAP_ON
    UVmap.xy = v.texcoord.xy * unity_LightmapST.xy + unity_LightmapST.zw;
    UVmap.zw = 0;
    // Sample light probe for Dynamic objects only (no static or dynamic lightmaps)
    #elif UNITY_SHOULD_SAMPLE_SH
        #ifdef VERTEXLIGHT_ON
            // Approximated illumination from non-important point lights
    UVmap.rgb = Shade4PointLights (
                unity_4LightPosX0, unity_4LightPosY0, unity_4LightPosZ0,
                unity_LightColor[0].rgb, unity_LightColor[1].rgb, unity_LightColor[2].rgb, unity_LightColor[3].rgb,
                unity_4LightAtten0, posWorld, normalWorld);
        #endif

    UVmap.rgb = ShadeSHPerVertex (normalWorld, UVmap.rgb);
    #endif

    #ifdef DYNAMICLIGHTMAP_ON
    UVmap.zw = v.texcoord2.xy * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
    #endif
...
inline FragmentCommonData FragmentSetup (inout float4 i_tex, float3 i_eyeVec, half3 i_viewDirForParallax, float4 tangentToWorld[3], float3 i_posWorld, half facing = 1)
{
    ...
    o.viewDirWS = NormalizePerPixelNormal(i_eyeVec);
...
#define FRAGMENT_SETUP(x) FragmentCommonData x = \
    FragmentSetup(IN.texcoord, IN.viewDirWS, IN_VIEWDIR4PARALLAX(IN), IN.tangentToWorldAndPackedData, IN_WORLDPOS(IN), facing);
...
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0
    ...
    half3 color =   diffColor * (data.bakedGI + light.color * diffuseTerm)
                    + specularTerm * light.color * FresnelTerm (specColor, lh)
                    + _FurScatterColor * (nl*0.5 + 0.5) * FabricScatterFresnelLerp(nv, _FurScatterScale);
    return half4(color, 1);
}
half4 FABRIC_BRDF_PBS (half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
    float3 normal, float3 viewDir,
...
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0
    ...
    half3 color =   diffColor * data.bakedGI + (diffColor *(light.color * diffuseTerm)
                    + specularTerm * light.color * FresnelTerm (specColor, lh)
                    + _FurScatterColor * (nl*0.5 + 0.5) * FabricScatterFresnelLerp(nv, _FurScatterScale)) * lightAttenuation;
    return half4(color, 1);
}
#endif 
